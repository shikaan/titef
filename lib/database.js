...
class Database extends EventEmitter {
  ...
  createRecordset(path, silent) {
    ...
    const obs = new PerformanceObserver((items) => {
      meta.duration = Math.ceil(items.getEntries()[0].duration) / 1000;
  ...
  closeRecord(path) {
    setImmediate(() => {
      ...
      if (isLastRecord) {
        ...
        /**
         * This will trigger the observer we defined in createRecordset which
         * is supposed to launch the event EVENT.DATABASE.PROCESS.ENDED
         */
        performance.measure('Duration', `${path}:init`, `${path}:end`);
