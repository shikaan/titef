...
const printSuite = (suite, suiteTitle, prefix = '') => {
  ...
  if (!suite.meta.silent) {
    print(
      TEXT_FORMAT.BOLD,
      `${prefix}${suiteTitle.toUpperCase()}`,
      TEXT_FORMAT.RESET,
      TEXT_FORMAT.YELLOW,
      ` (${suite.meta.duration.toFixed(2)}s)`,
    );
  }
    ...
    .forEach(([specTitle, spec]) => {
      ...
      if (!isSilent) {
        ...
        if (isSuite) {
          ...
          const timing = spec.duration > 1 ? [TEXT_FORMAT.YELLOW, ` (${spec.duration.toFixed(2)}s)`] : [];

          if (spec.result === RESULT.SUCCESS) {
            print(TEXT_FORMAT.GREEN, `${prefix} ✔ ${specTitle}`, ...timing);
          } else if (spec.result === RESULT.IGNORED) {
            print(TEXT_FORMAT.YELLOW, `${prefix} - ${specTitle}`, ...timing);
          } else if (spec.result === RESULT.PENDING) {
            print(TEXT_FORMAT.GREY, `${prefix} . ${specTitle}`, ...timing);
          } else {
            print(TEXT_FORMAT.RED, `${prefix} ✕ ${specTitle}`, ...timing);
...
const printDatabase = async (database) => {
  ...
  Object.entries(database).forEach(([title, suite]) => {
    ...
    print('\n  Duration: ', TEXT_FORMAT.YELLOW, `${suite.meta.duration.toFixed(2)}s`);
  });
